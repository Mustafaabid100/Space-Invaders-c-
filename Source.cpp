...
int main() {
    PlaySound(TEXT("strangerthings.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
    while (extremeloop) {
        while (menu) {
            if (scoreboardclear) {
                system("cls");
            ...
            drawText(60, 520, 110, 0, 255, 0, "SPACE INVADERS");
        ...
        while (game) {
            ...
            // diplaying  highe score at scoreboard
            ifstream myrecord;
            myrecord.open("highscores.txt");
            int n, i = 0;
            while (myrecord >> n) {
                displayhighscores[i] = n;//5  HIGH SCORES IN ORDER
                i++;
            }
            myrecord.close();
            int displayhighestscore = displayhighscores[0];
                ...
                for (int row = 0; row < rows; ++row) {
                    for (int col = 0; col < cols; ++col) {
                        if (death[row][col]) {
                            ...
                            X[row][col] -= 4; // Move alien left by 10 pixels
